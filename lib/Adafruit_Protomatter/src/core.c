...
/* Optional instrumentation, compiled in only by env:s3_diag (-DPM_DIAG_COUNTERS).
   `elapsed` is the per-row reading that drives bitZeroPeriod, and on the S3 it
   is an estimate, so watching its distribution is how the flicker described in
   FORK.md was diagnosed. Written from the row ISR and read from a task: plain
   32-bit words, so a torn read is impossible and a missed update costs nothing.
   Kept because phase 4.2 moves rendering to its own task and will want to
   re-measure. */
#ifdef PM_DIAG_COUNTERS
volatile uint32_t _PM_diagRows = 0;
volatile uint32_t _PM_diagSpikes = 0;
volatile uint32_t _PM_diagMaxElapsed = 0;
volatile uint32_t _PM_diagMaxPeriod = 0;
volatile uint32_t _PM_diagMinPeriod = 0;
volatile uint32_t _PM_diagClamped = 0;
#endif
...
#ifdef PM_DIAG_COUNTERS
    _PM_diagRows++;
    if (elapsed > _PM_diagMaxElapsed) _PM_diagMaxElapsed = elapsed;
    if (elapsed > core->minPeriod + (core->minPeriod >> 1)) _PM_diagSpikes++;
    _PM_diagMinPeriod = core->minPeriod;
#endif
...
#ifdef _PM_periodCeiling
    // LOCAL CHANGE -- see FORK.md. The floor above stops the period being too
    // short to shift a row out. Nothing upstream stops it being too long, and
    // on a backend that estimates `elapsed` rather than measuring it, one bad
    // reading stretches bitplane N by (period << N). Only defined by backends
    // where that estimate exists.
    {
      const uint32_t ceiling = _PM_periodCeiling(core);
#ifdef PM_DIAG_COUNTERS
      if (core->bitZeroPeriod > _PM_diagMaxPeriod) _PM_diagMaxPeriod = core->bitZeroPeriod;
#endif
      if (core->bitZeroPeriod > ceiling) {
        core->bitZeroPeriod = ceiling;
#ifdef PM_DIAG_COUNTERS
        _PM_diagClamped++;
#endif
      }
