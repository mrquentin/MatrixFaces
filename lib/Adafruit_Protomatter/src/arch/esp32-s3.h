...
// LOCAL CHANGE 3 of 3 -- see FORK.md.
// Defining this makes core.c bound the adaptive bitplane period from above as
// well as below. It is defined ONLY here, for this backend, because only here
// is `elapsed` an estimate rather than a measurement: on the SAMD51 it is a
// real hardware count of how long issuing a row took, and capping it there
// could start the next row before the current one has finished.
#define _PM_periodCeiling(core) ((core)->minPeriod + ((core)->minPeriod >> 1))
...
// LOCAL CHANGE 2 of 3 -- see FORK.md.
// gdma_start() is called from the row ISR on every scanline (~3400/s), and in
// the Arduino core's prebuilt ESP-IDF it lives in flash:
// CONFIG_GDMA_CTRL_FUNC_IN_IRAM is not set. Each call therefore risks an
// instruction-cache miss that stalls the CPU on a SPI fetch, and it takes a
// spinlock on the way through. Since the ISR turns LED output off first, that
// delay lights the previous row too long.
//
// The instruction cache is shared between both cores, which is why neither
// raising the interrupt priority nor moving WiFi (or every other task) to the
// other core removed this -- both were measured, and neither moved the number.
// Doing the two register writes inline, from IRAM, does: worst-case row time
// 2142 -> 1981 ticks, and the refresh rate rises with it (335 -> 361 rows per
// 100 ms) because the ISR stops waiting on flash.
static int _PM_dmaChannelId = -1;
...
IRAM_ATTR static void blast_byte(Protomatter_core *core, uint8_t *data) {
  ...
  desc.buffer = data;
  // Inlined gdma_start(); see _PM_dmaChannelId above. Falls back to the driver
  // call if the id was never obtained, so a failure degrades to upstream
  // behaviour rather than to a dead display.
  if (_PM_dmaChannelId >= 0) {
    GDMA.channel[_PM_dmaChannelId].out.link.addr = (uint32_t)&desc;
    GDMA.channel[_PM_dmaChannelId].out.link.start = 1;
  } else {
...
static void _PM_timerInit(Protomatter_core *core) {
  ...
  // Needed by blast_byte() to address the channel directly; see above.
  gdma_get_channel_id(dma_chan, &_PM_dmaChannelId);
