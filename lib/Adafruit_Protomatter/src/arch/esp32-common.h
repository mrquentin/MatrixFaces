...
#if ESP_IDF_VERSION < ESP_IDF_VERSION_VAL(5, 0, 0)
  // LOCAL CHANGE 1 of 3 -- see FORK.md.
  // Upstream calls timerAttachInterrupt(), which passes intr_alloc_flags = 0
  // and so lands the row handler on the default (lowest) interrupt level,
  // alongside WiFi's. The handler disables LED output as its very first act,
  // so every microsecond it is kept waiting is a microsecond the previous row
  // stays lit -- and on the shortest bitplane that is a large proportional
  // overshoot, i.e. a visibly bright row. Measured on a MatrixPortal S3 with
  // WiFi associated: worst-case row time 5834 timer ticks against a 488-tick
  // floor; at level 3 that falls to 2142.
  //
  // Priority only. ESP_INTR_FLAG_IRAM additionally asserts the whole path is
  // safe to run with the flash cache disabled, and it is not -- blast_byte()
  // reaches IDF code that lives in flash. Asking for it reboot-loops the board.
  timerAttachInterruptFlag(core->timer, &_PM_esp32timerCallback, true,
